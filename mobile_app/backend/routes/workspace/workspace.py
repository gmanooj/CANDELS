...
def create_workspace_task():
    ...
    try:
        ...
        leader_check = db.session.execute(
            text("""
                SELECT t.leader_code FROM teams t
        ...
        # Insert task assignment notification
        if assigned_to:
            db.session.execute(
                text("""
                    INSERT INTO system_notifications (recipient_code, sender_code, team_code, notification_type, message_text, status)
                    VALUES (:rc, :sc, :tc, 'Task_Assignment', :msg, 'Pending')
                """),
                {
                    "rc": assigned_to,
                    "sc": leader_code,
                    "tc": team_code,
                    "msg": f"assigned you a task: '{title}'"
                }
            )
    ...
    try:
        ...
        if status is not None and status != old_status:
            ...
            if assigned_to and assigned_to != '':
                leader_query = db.session.execute(
                    text("SELECT leader_code FROM teams WHERE team_code = :code"),
                    {"code": team_code}
                ).fetchone()
                leader_code = leader_query[0] if leader_query else 'system'
...
def get_workspace_documents():
    ...
    try:
        query = text("""
            SELECT d.id, d.document_name, d.document_url, d.created_at, u.first_name, u.last_name, d.content_buffer, d.file_path, d.uploaded_by
...
def get_workspace_chat_messages():
    ...
    try:
        ...
        db.session.execute(text("""
            CREATE TABLE IF NOT EXISTS workspace_chat_views (
                id INT AUTO_INCREMENT PRIMARY KEY,
                message_id INT NOT NULL,
                viewer_code VARCHAR(255) NOT NULL,
                viewer_name VARCHAR(255) NOT NULL,
                viewed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                UNIQUE KEY unique_view (message_id, viewer_code)
            )
        """))
...
def delete_workspace_chat_message():
    ...
    try:
        ...
        # Emit delete event so clients can remove it from view
        team_code = data.get('team_code')
        if team_code:
    ...
    try:
        query = text("""
            SELECT i.id, i.image_url, i.title, i.category, i.created_at, u.first_name, u.last_name,
                   i.grade_score, i.grading_feedback, i.graded_at, gr.first_name, gr.last_name, i.uploaded_by
...
@workspace_bp.route('/git/sync', methods=['POST'])
    ...
    try:
        ...
        if setting:
            ...
            for idx, git_link in enumerate(links):
                ...
                # Check remote commit
                try:
                    res_remote = subprocess.run(["git", "ls-remote", git_link, "HEAD"], capture_output=True, text=True, timeout=5)
                ...
                # Check local commit if cloned
                if os.path.exists(repo_dir) and os.path.exists(os.path.join(repo_dir, ".git")):
                    try:
                        res_local = subprocess.run(["git", "rev-parse", "HEAD"], cwd=repo_dir, capture_output=True, text=True, timeout=5)
...
def pull_external_git():
    ...
    try:
        ...
        # Run Git updates: fetch and hard reset to match remote state
        try:
            subprocess.run(["git", "fetch", "--all"], cwd=repo_dir, capture_output=True, check=True, timeout=15)
            
            # Reset to origin/main or origin/master
            res_reset = subprocess.run(["git", "reset", "--hard", "origin/main"], cwd=repo_dir, capture_output=True, text=True, timeout=10)
            if res_reset.returncode != 0:
                res_reset_master = subprocess.run(["git", "reset", "--hard", "origin/master"], cwd=repo_dir, capture_output=True, text=True, timeout=10)
                if res_reset_master.returncode != 0:
                    # Fallback to origin/HEAD
                    subprocess.run(["git", "reset", "--hard", "origin/HEAD"], cwd=repo_dir, check=True, timeout=10)
        except Exception as e:
            return jsonify({"error": f"Failed to pull changes from Git remote: {str(e)}"}), 500

        # Log action to activity heartbeat
