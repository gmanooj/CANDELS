...
settings_bp = Blueprint('settings_bp', __name__, url_prefix='/api/v1/user')
...
def check_and_migrate_db():
    ...
    try:
        ...
        db.session.commit()
        print("[SUCCESS] Settings database migrations executed successfully.")
    except Exception as e:
        db.session.rollback()
...
@settings_bp.route('/settings/sessions/revoke', methods=['POST'])
def revoke_user_session():
    ...
    try:
        # Check if the session belongs to this user and is active
        session_row = db.session.execute(text("""
            SELECT user_code, token_hash FROM user_sessions WHERE id = :id AND is_active = 1
