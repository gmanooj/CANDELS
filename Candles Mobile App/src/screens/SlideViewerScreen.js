...
export default function SlideViewerScreen({ navigation }) {
        ...
        return (
            <View style={[styles.mainLayout, { backgroundColor: theme.colors.backgroundStart, justifyContent: 'center' }]}>
                <Text style={{ color: theme.colors.danger, textAlign: 'center' }}>
                    Access Denied: Please log in offline first.
                </Text>
            </View>
        );
    }

    // Load slide markdown source from local README.md
    const readmeFile = offlineWorkspaces.files.find(f => f.path === 'README.md');
    const readmeContent = readmeFile ? readmeFile.content : '# Presentation\n---\n## No Content Available';
